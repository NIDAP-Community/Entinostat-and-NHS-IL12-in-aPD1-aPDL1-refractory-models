normalized_counts <- function(filtered_counts_1, ccbr1144_Metadata) {
    #image: png
    imageType = "png"
    ...
    df <- filtered_counts_1
    
    samples_for_deg_analysis = c("Gene","S1_1","S1_2","S1_3","S2_1","S2_2","S2_3","S3_1","S3_2","S3_3","S4_1","S4_2","S4_4","S4_6")
    ...
    colnames(design) <- str_replace_all(colnames(design), c("Group"), "")
    ...
    samples_to_include = c("Gene","S1_1","S1_2","S1_3","S2_1","S2_2","S2_3","S3_1","S3_2","S3_3","S4_1","S4_2","S4_4","S4_6")
    ...
    pca.df$celltype <- Sampinfo$Group
    pca.df$sample <- Sampinfo$Label
    ...
    if (length(unique(Sampinfo$Group)) > length(plotcolors)) {
        ...
        k=length(unique(Sampinfo$Group))-length(plotcolors)
 ...
 df.m %>% mutate(colgroup = Sampinfo[variable,"Group"]) -> df.m
...
g2 = ggplot(df.m, aes(x=value, group=variable)) + 
    ...
    scale_colour_manual(values=altplotcolors)
     ...
     df.m$variable = Sampinfo[df.m$variable,"Label"]
...
make_heatmap <- function(df, metadata,plotcolors) {
  ...
  idx = as.factor(targetfile[rownames(m),"Group"])
  col = plotcolors
  cols <- col[idx]
...
var_filtered_counts_1<-readRDS("var_filtered_counts_1.rds")
var_ccbr1144_Metadata<-readRDS("var_ccbr1144_Metadata.rds")
invisible(graphics.off())
var_normalized_counts<-normalized_counts(var_filtered_counts_1,var_ccbr1144_Metadata)
invisible(graphics.off())
saveRDS(var_normalized_counts,"var_normalized_counts.rds")
