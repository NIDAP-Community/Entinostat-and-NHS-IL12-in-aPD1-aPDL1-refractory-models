DEG_COMBO_NHS <- function(filtered_counts_1, ccbr1144_Metadata) {
    ...
    samples_for_deg_analysis = c("S1_1","S1_2","S1_3","S2_1","S2_2","S2_3","S3_1","S3_2","S3_3","S4_1","S4_2","S4_4","S4_6")
    targetfile <- ccbr1144_Metadata
    contrasts_param<-c("COMBO-NHS")
    sample_name_column_param<-"Sample"
    ordered_covariates=c("Group")
    return_mean_and_sd_param<-TRUE
    return_normalized_expression<-TRUE
    contrast_variable_param<-"Group"
    ...
    if(return_normalized_expression){
...
print("template_function_DEG_COMBO_NHS.R #########################################################################")
...
var_DEG_COMBO_NHS<-DEG_COMBO_NHS(var_filtered_counts_1,var_ccbr1144_Metadata)
invisible(graphics.off())
saveRDS(var_DEG_COMBO_NHS,"var_DEG_COMBO_NHS.rds")
