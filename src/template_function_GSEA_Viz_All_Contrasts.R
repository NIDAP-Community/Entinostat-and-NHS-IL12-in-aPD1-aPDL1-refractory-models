GSEA_Viz_All_Contrasts <- function(DEG_COMBO_PBS, GSEA_Filter_All_Contrasts, normalized_counts, ccbr1144_Metadata, msigdb_v6_2_with_orthologs) {

    graphics.off()

## LIBRARIES ====
    library(tibble); library(dplyr); library(tidyr)
    library(ggplot2); library(patchwork); library(ComplexHeatmap); library(colorspace); library(RColorBrewer)
    library(fgsea)
...
## PARAMETERS
    ...
    plot_limit = "ALL"
    topline = 'coordinate'
    pathways = c("KEGG_JAK_STAT_SIGNALING_PATHWAY","KEGG_ANTIGEN_PROCESSING_AND_PRESENTATION","GOLDRATH_NAIVE_VS_EFF_CD8_TCELL_UP","HALLMARK_INFLAMMATORY_RESPONSE","KEGG_CYTOKINE_CYTOKINE_RECEPTOR_INTERACTION","HALLMARK_INTERFERON_GAMMA_RESPONSE","GO_ACTIVATION_OF_IMMUNE_RESPONSE")
    geneid = "Gene"
    geneid_gex = "Gene"
    contrasts = c()
    species = "Mouse"

    genescore_df <- DEG_COMBO_PBS
    gex_df <- normalized_counts
    metadata_df <- ccbr1144_Metadata
    gsea_df <- as.data.frame(GSEA_Filter_All_Contrasts)
    pathway_db <- msigdb_v6_2_with_orthologs

    print(dim(genescore_df)) 
    print(dim(gex_df)) 
    print(dim(metadata_df)) 
    print(dim(gsea_df)) 
    print(dim(pathway_db)) 
...
if (grepl("LE", which_plot)) {
    ...
    samples_to_include = setdiff(c("S1_1","S1_2","S1_3","S2_1","S2_2","S2_3","S3_1","S3_2","S3_3","S4_1","S4_2","S4_4","S4_6"), geneid)
...
for ( i in 1:length(gsea_list) ) {
    ...
    plotES = gg.plotES(ranks=genescore_df, gsea=gsea, ntop=10, add_top=TRUE, cex_top=3, image_size='full', gset=gset, line_top=topline, ES_colo = 'ES sign')
    gsea_out <- plotES[[3]]
    ...
    if (which_plot == "ES+RNK+LE") {
        ...
        # clustering        
        clustOrder <- prep.clustOrder( df=gex_trans, linkage='complete', distance='Euclidean', way='rows' )
...
png(filename="GSEA_Viz_All_Contrasts.png", width=(pageWidth+5)*300, height=(pageHeight+5)*300, units="px", pointsize=4, bg="white", res=300, type="cairo") 
print(preview)
...
print("template_function_GSEA_Viz_All_Contrasts.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
var_DEG_COMBO_PBS<-readRDS("var_DEG_COMBO_PBS.rds")
var_GSEA_Filter_All_Contrasts<-readRDS("var_GSEA_Filter_All_Contrasts.rds")
var_normalized_counts<-readRDS("var_normalized_counts.rds")
var_ccbr1144_Metadata<-readRDS("var_ccbr1144_Metadata.rds")
var_msigdb_v6_2_with_orthologs<-readRDS("var_msigdb_v6_2_with_orthologs.rds")
invisible(graphics.off())
var_GSEA_Viz_All_Contrasts<-GSEA_Viz_All_Contrasts(var_DEG_COMBO_PBS,var_GSEA_Filter_All_Contrasts,var_normalized_counts,var_ccbr1144_Metadata,var_msigdb_v6_2_with_orthologs)
invisible(graphics.off())
saveRDS(var_GSEA_Viz_All_Contrasts,"var_GSEA_Viz_All_Contrasts.rds")
