GSEA_Filter_All_Contrasts <- function(GSEA_Preranked_All_Contrasts, msigdb_v6_2_with_orthologs ) {
...
## dataset
    gsea <- GSEA_Preranked_All_Contrasts
...
## apply filters
    ...
    if ( ! is.null(genes) ) {
        ...
        if (filterInput_bySet == 'Leading Edge (LE)') { 
            ...
            index_pathway = lapply(genes, function(x) grep(paste0("\\b\\Q", x, "\\E\\b"), gsea_filtered$leadingEdge))
            ...
            index_pathway = lapply(genes, function(x) grep(paste0("\\b\\Q", x, "\\E\\b"), gsea_filtered$inPathway))
            found = sapply(index_pathway, function(x) {length(x) > 0})
            index_pathway = Reduce(union, index_pathway)
            gsea_filtered <- gsea_filtered %>% dplyr::slice(index_pathway) 
...
filterInput_byTop = tolower(filterInput_byTop)

if ( filterInput_byTop == "all" ) {
    ...
    filter.message(filter="Top significant filter", condition=sprintf("up to p-value rank of %s per contrast and collection", filterInput_keep), output=gsea_filtered)
...
print("template_function_GSEA_Filter_All_Contrasts.R #########################################################################")
library(plotly);library(ggplot2);library(jsonlite);
var_GSEA_Preranked_All_Contrasts<-readRDS("var_GSEA_Preranked_All_Contrasts.rds")
var_msigdb_v6_2_with_orthologs<-readRDS("var_msigdb_v6_2_with_orthologs.rds")
invisible(graphics.off())
var_GSEA_Filter_All_Contrasts<-GSEA_Filter_All_Contrasts(var_GSEA_Preranked_All_Contrasts,var_msigdb_v6_2_with_orthologs)
invisible(graphics.off())
saveRDS(var_GSEA_Filter_All_Contrasts,"var_GSEA_Filter_All_Contrasts.rds")
